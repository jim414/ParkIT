ParkIt is a Python script for parking a telescope to an azimuth and altitude that is relative to the home position of the scope. As an example, if your scope is in your garage and home is pointing at some arbitrary angle, the script assumes that it is pointing north. All azimuth settings are relative to its home position. So to rotate the scope to a relative east, enter 90 as the azimuth, etc.   This can be used to point the telescope in a vertical position for taking flats or a horizontal position for storage in an observatory shelter or any other position desired as long as the coordinates are not below the horizon. It does this by converting the horizontal coordinates, azimuth and altitude to JNOW equatorial coordinates, RA and DEC, that can be entered into your mount control software such as the ASIAIR. The script can be capable of be run on both Windows and Mac systems that host Python. It has also been tested with the Juno Python app on iOs successfully although the set up is a bit more complicated. It may work on Android devices that have a Python environment that allows the adding of packages.
